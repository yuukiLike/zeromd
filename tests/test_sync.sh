...
_setup_bare_remote() {
    ...
    git -C "$CC_MD_TEST_VAULT" push -u origin main --quiet 2>/dev/null
    echo "$bare_remote"
}
...
test_heartbeat_written() {
    ...
    if [ ! -f "$CC_MD_STATE_DIR/last-heartbeat" ]; then
    ...
    local hb
    hb=$(cat "$CC_MD_STATE_DIR/last-heartbeat")
    local after
    after=$(date +%s)
...
test_no_changes_skip() {
    ...
    if [ -f "$CC_MD_STATE_DIR/last-sync" ]; then
        _CURRENT_TEST_FAILED=1
...
test_rebase_detected() {
    mkdir -p "$CC_MD_TEST_VAULT/.git/rebase-merge"
    ...
    local err
    err=$(cat "$CC_MD_STATE_DIR/last-error")
...
test_consecutive_failures() {
    ...
    bash "$SYNC_CMD" >/dev/null 2>&1 || true
    local count1
    count1=$(cat "$CC_MD_STATE_DIR/consecutive-failures")
    assert_eq "1" "$count1" "first failure should be 1"

    bash "$SYNC_CMD" >/dev/null 2>&1 || true
    local count2
    count2=$(cat "$CC_MD_STATE_DIR/consecutive-failures")
...
test_success_clears_error() {
    # 预先写入错误状态
    echo "old error" > "$CC_MD_STATE_DIR/last-error"
    echo "3" > "$CC_MD_STATE_DIR/consecutive-failures"
    ...
    # 制造变更
    echo "test content" > "$CC_MD_TEST_VAULT/test.md"
    ...
    # last-error 应被删除
    if [ -f "$CC_MD_STATE_DIR/last-error" ]; then
        _CURRENT_TEST_FAILED=1
    ...
    local failures
    failures=$(cat "$CC_MD_STATE_DIR/consecutive-failures")
